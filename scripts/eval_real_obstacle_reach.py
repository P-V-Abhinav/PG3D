...
def _constraints_for_episode(
    ...
    *,
    ...
    if hasattr(unwrapped, "obstacle"):
        obstacle_pos = unwrapped.obstacle.pose.p.cpu().numpy()[0]
    else:
        # Fallback if somehow not our custom env
        obstacle_pos = np.array([0.0, 0.0, 0.15], dtype=np.float32)

    constraint = AvoidRegion(
        region=BoxRegion(
            center=obstacle_pos,
            half_extents=np.array([0.03, 0.03, 0.15], dtype=np.float32)
        ),
        margin=float(args.avoid_margin),
        weight=float(args.avoid_weight),
        name="real_obstacle_avoid_region",
