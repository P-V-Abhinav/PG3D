...
import pg3d.policies.dp3.goal_markers
_original_goal_marker_offsets = pg3d.policies.dp3.goal_markers.goal_marker_offsets
def _tilted_goal_marker_offsets(*args, **kwargs):
    import numpy as np
    offsets = _original_goal_marker_offsets(*args, **kwargs)
    tilted = np.zeros_like(offsets)
    tilted[:, 0] = offsets[:, 2]
    tilted[:, 1] = offsets[:, 1]
    tilted[:, 2] = -offsets[:, 0]
    return tilted
pg3d.policies.dp3.goal_markers.goal_marker_offsets = _tilted_goal_marker_offsets
...
def save_rerun_timeline(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError("Rerun export requires: uv sync --extra cu129 --extra maniskill --extra viz --group dev --group notebooks") from exc
    ...
    if constraints:
        ...
        for visual in avoid_region_line_visuals(constraints):
            rr.log(f"world/constraints/{visual.name}", rr.LineStrips3D(visual.line_strips, colors=visual.color), static=True)
    ...
    for step_idx, entry in enumerate(timeline):
        ...
        policy_pts = np.asarray(entry["point_cloud"], dtype=np.float32).copy()
        ...
        if np.all(np.isfinite(target)):
            ...
            policy_pts = insert_goal_marker_points(policy_pts, target, num_points=192, radius=0.055)
            rr.log("world/goal", rr.Points3D(target.reshape(1, 3), colors=[0, 255, 0]))
        
        if policy_pts.size:
            rr.log("world/point_cloud", rr.Points3D(policy_pts, colors=[180, 180, 180]))
            robot_mask = np.asarray(entry["robot_mask"], dtype=bool)
            if len(robot_mask) == len(policy_pts):
                robot_points = policy_pts[robot_mask]
        ...
        if decisions is not None:
            ...
            if active_decision is not None and getattr(active_decision, "result", None) is not None:
                ...
                if rejected_paths:
                    rr.log("world/predicted_trajectories/rejected", rr.LineStrips3D(rejected_paths, colors=[100, 100, 100, 128], radii=0.001))
                ...
                if selected_path.ndim == 2 and selected_path.shape[0] >= 2 and selected_path.shape[1] == 3:
                    rr.log("world/predicted_trajectories/selected", rr.LineStrips3D([selected_path], colors=[0, 255, 255, 255], radii=0.003))
    rr.disconnect()
