...
def main(argv: list[str] | None = None) -> int:
    ...
    _resolve_goal_marker_config(args)
    ...
    # Shuffle validation with a fixed generator so the capped `max_val_batches`
    # window samples a diverse spread of the val set each eval (reproducibly)
    # instead of always scoring the same first few batches.
    ...
    for step in range(1, args.max_steps + 1):
        ...
        if args.use_goal_encoder:
            metrics["train/goal_xyz_mean"] = float(
                batch["obs"]["goal_xyz"].detach().mean().cpu()
            )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "trailing cloud slots treated as the goal-marker branch. Default: read "
            "point_cloud_saliency.goal_marker_points from the dataset metadata so training "
            "matches the bake exactly. Override only to deliberately differ from the dataset."
        ),
    )
    parser.add_argument(
        "--goal-marker-radius",
        ...
        default=None,
        help=(
            "goal-marker radius. Default: read point_cloud_saliency.goal_marker_radius from the "
            "dataset metadata so training matches the bake exactly."
        ),
    )
    parser.add_argument(
        "--goal-marker-style",
    ...
    parser.add_argument(
        ...
        default=True,
        help="Add explicit goal_xyz / ee_position conditioning in addition to goal markers.",
    )
    parser.add_argument(
        "--log-goal-encoder-shapes",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Print goal_xyz, goal_rel, goal_feat, and fused feature shapes once.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="minmax",
        help=(
            "action normalization: 'minmax' maps the action range to [-1, 1] so the "
            "scheduler's clip_sample clamp does not truncate action tails; 'standardize' "
            "uses zero-mean/unit-std. Observation fields always use standardize."
        ),
    )
    ...
    parser.add_argument(
        "--checkpoint-rollout-resample-per-step",
        action=argparse.BooleanOptionalAction,
        default=True,
        help=(
            "Draw a different diverse episode subset at each checkpoint (seed += step) so "
            "rollout accuracy is estimated across the whole dataset over a run. Disable to keep "
            "a fixed episode set across all checkpoints for strict step-to-step comparison."
        ),
    )
    parser.add_argument("--checkpoint-rollout-max-steps", type=int, default=150)
    ...
    if args.goal_marker_points is not None and args.goal_marker_points < 0:
        raise ValueError("--goal-marker-points must be non-negative")
    if args.goal_marker_radius is not None and args.goal_marker_radius < 0:
...
def _resolve_goal_marker_config(args: argparse.Namespace) -> None:
    """Align goal-marker count/radius with the dataset's baked saliency layout.

    The encoder splits off the trailing ``goal_marker_points`` cloud slots as the
    goal branch and the dataset loader OVERWRITES exactly those slots at train
    time. If this count does not equal the number of goal slots the dataset
    writer baked, training either leaks baked markers into the PointNet scene
    branch (count too small) or destroys real robot/scene points (count too
    large). Reading the values from ``point_cloud_saliency`` guarantees a match.
    """
    saliency: dict[str, Any] = {}
    try:
        saliency = dict(load_reach_metadata(args.dataset).get("point_cloud_saliency") or {})
    except Exception as exc:  # noqa: BLE001 - metadata is advisory; fall back to defaults
        print(
            f"warning: could not read dataset metadata for goal-marker alignment "
            f"({type(exc).__name__}: {exc}); using fallback defaults",
            flush=True,
        )
    ...
    if args.goal_marker_points is None:
        args.goal_marker_points = (
            int(baked_points) if baked_points is not None else DEFAULT_GOAL_MARKER_POINTS
        )
        points_src = "metadata" if baked_points is not None else "fallback-default"
    else:
        points_src = "cli-override"
    if args.goal_marker_radius is None:
        args.goal_marker_radius = (
            float(baked_radius) if baked_radius is not None else DEFAULT_GOAL_MARKER_RADIUS
        )
    ...
    print(
        ...
        f"points={args.goal_marker_points} ({points_src}) "
        f"radius={args.goal_marker_radius} ({radius_src})",
        flush=True,
    )
    if baked_points is not None and int(baked_points) != int(args.goal_marker_points):
        print(
            f"WARNING: training goal_marker_points={args.goal_marker_points} != dataset bake "
            f"{int(baked_points)}. The encoder will overwrite the trailing "
            f"{args.goal_marker_points} slots, which does NOT match the {int(baked_points)} goal "
            "slots the dataset baked. This corrupts the cloud unless intentional.",
            flush=True,
        )
    radius_mismatch = baked_radius is not None and abs(
        float(baked_radius) - float(args.goal_marker_radius)
    ) > 1e-9
    if radius_mismatch:
        print(
            f"WARNING: training goal_marker_radius={args.goal_marker_radius} != dataset bake "
            f"{float(baked_radius)}.",
            flush=True,
        )
...
def _log_checkpoint_rollouts(
    ...
    *,
    ...
    accuracy_matrix = _rollout_accuracy_table(wandb, summaries, step=step)
    run.log(
        {
            **videos,
            "rollout/source": (
            ...
            # `reach_accuracy` is the headline accuracy metric (fraction of
            # rollouts that reached the goal within the success threshold).
            "rollout/reach_accuracy": success_rate,
            "rollout/accuracy_matrix": accuracy_matrix,
...
def _rollout_accuracy_table(wandb: Any, summaries: list[dict[str, Any]], *, step: int) -> Any:
    """Build a per-rollout accuracy matrix as a sortable/filterable W&B table."""
    table = wandb.Table(
        columns=[
            "step",
            "output_index",
    ...
    for summary in summaries:
        ...
        final_distance = (
            ...
            if final_distance is not None and math.isfinite(float(final_distance))
            else None
        )
        table.add_data(
            step,
            summary.get("episode"),
            summary.get("source"),
            summary.get("dataset_episode_index"),
            summary.get("seed"),
            bool(summary.get("success")),
            final_distance,
            summary.get("min_distance"),
            summary.get("first_success_step"),
            summary.get("steps"),
        )
    return table
...
def _checkpoint_rollout_metadata_and_specs(
    ...
    *,
    ...
    # Resample a different diverse episode subset at each checkpoint so accuracy
    # is estimated across the whole dataset over a run, not on a fixed few seeds.
    selection_seed = args.checkpoint_rollout_selection_seed
    if args.checkpoint_rollout_resample_per_step:
        selection_seed = selection_seed + step
    specs = (
        select_random_dataset_rollout_specs(
