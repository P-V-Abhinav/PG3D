...
def _select_decision(
    *,
    ...
    search_entry = _copy_entry(current_entry)
    search_window = [_copy_entry(w) for w in obs_window]
    
    first_step_least_bad = None
    first_step_least_bad_result = None
    ...
    for depth in range(max_search_depth):
        ...
        for retry in range(max_retries):
            ...
            elif depth == 0 and retry == max_retries - 1 and not result.selected.feasible:
                first_step_least_bad = result.action_chunk
                first_step_least_bad_result = result
            ...
            if result.selected.feasible:
                ...
                for step_idx in range(rollout.action_chunk.horizon):
                    search_entry = world_model_entry_from_rollout_step(
                        rollout,
                        step_idx,
                        previous_entry=search_entry,
                        crop_config=crop_config,
                        goal_thresh=goal_thresh,
                    )
                    search_window = append_obs_window(
                        search_window,
                        search_entry,
    ...
    if found_path_to_goal and len(safe_chunks) > 0:
        ...
        return EvalDecisionSummary(
            selected_chunk=first_step_least_bad,
            result=first_step_least_bad_result,
            candidate_feasible=total_candidate_feasible,
            candidate_total=total_candidate_total,
            selection_reason="least_bad_fallback",
        )
...
def _constraints_for_episode(
    ...
    *,
    ...
    # Filter: remove goal marker points
    ...
    # Filter: remove start marker points
