...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.viz_trajectories:
        return _visualize_trajectory_families(args)
    try:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "print per-family lateral-axis angle (degrees) and path ratios for each "
            "waypoint-sampling attempt; useful for debugging trajectory diversity"
        ),
    )
    parser.add_argument(
        "--viz-trajectories",
        ...
        default=False,
        help=(
            "visualize sampled trajectory families for one start/goal pair with matplotlib "
            "and exit; does not require ManiSkill. Use --viz-start/--viz-goal to set points, "
            "--viz-seeds to overlay multiple random seeds, --viz-save to write to file."
        ),
    )
    parser.add_argument(
        "--viz-start",
        type=float,
        nargs=3,
        default=[0.20, -0.18, 0.20],
        metavar=("X", "Y", "Z"),
        help="TCP start point for --viz-trajectories (metres)",
    )
    parser.add_argument(
        "--viz-goal",
        type=float,
        nargs=3,
        default=[0.35, 0.22, 0.38],
        metavar=("X", "Y", "Z"),
        help="TCP goal point for --viz-trajectories (metres)",
    )
    parser.add_argument(
        "--viz-seeds",
        type=int,
        default=1,
        help="number of random seeds to overlay in --viz-trajectories (each gets unique colours)",
    )
    parser.add_argument(
        "--viz-save",
        type=Path,
        default=None,
        metavar="PATH",
        help="save --viz-trajectories plot to this file instead of opening an interactive window",
    )
    ...
    if args.curvature_std < 0:
        raise ValueError("--curvature-std must be non-negative")
    ...
    try:
        ...
        for ori_name, ori_quat in target_orientations.items():
            ...
            if goal_reachability is None:
                print(
                    f"[seed {seed}] orientation mode {ori_name} rejected: direct goal plan failed from sampled start",
                    flush=True,
                )
                continue
...
def _trajectory_variant_specs(
    ...
    # Asymmetric family layout (no +x/-x mirror twins).
    #
    # The previous layout used mirror pairs (left_wide/right_wide, upper_left/upper_right, ...)
    # whose (lateral, vertical) offsets cancelled in expectation. Because the detour family is
    # stripped before training, the diffusion policy sees identical observations for both twins
    # at the path-divergence point and averages their opposing scores to ~zero lateral motion --
    # a symmetric saddle that shows up at eval as the end-effector stalling/jerking partway to the
    # goal. Each family below has a distinct (lateral, vertical) direction AND magnitude so no two
    # are mirror images, and the set centroid is deliberately non-degenerate. Per-episode random
    # rotation of the lateral axis (see `_trajectory_basis`) further removes any global
    # straight-line average the policy could memorize.
    base_specs: list[TrajectoryFamilySpec] = [
        TrajectoryFamilySpec(0, "left_wide", -0.90, 0.12, (0.34, 0.68), 1.10),
        TrajectoryFamilySpec(1, "right_narrow", 0.52, -0.06, (0.40,), 1.05),
        TrajectoryFamilySpec(2, "upper_left", -0.62, 0.86, (0.30, 0.62), 1.15),
        TrajectoryFamilySpec(3, "upper_right", -0.80, -0.70, (0.34, 0.68), 1.10),
        TrajectoryFamilySpec(4, "lower_left", -0.42, -0.40, (0.35, 0.70), 1.05),
        TrajectoryFamilySpec(5, "lower_right", 0.30, -0.28, (0.46,), 1.00),
        TrajectoryFamilySpec(6, "upper_arc", 0.14, 1.10, (0.50,), 0.90),
        TrajectoryFamilySpec(7, "lower_arc", -0.12, -0.46, (0.55,), 0.80),
        TrajectoryFamilySpec(8, "high_loop", -0.34, 0.92, (0.30, 0.66), 0.95),
        TrajectoryFamilySpec(9, "low_loop", 0.88, -0.85, (0.50,), 1.00),
        ...
        TrajectoryFamilySpec(11, "extreme_detour", 0.82, 0.34, (0.30, 0.60), 1.00),
    ]
    if variants_per_reset <= len(base_specs):
        return base_specs[:variants_per_reset]
    # Additional families use golden-angle spacing in the (lateral, vertical) detour plane so
    # they remain distinct and non-mirror for any count, with magnitudes that walk between
    # moderate and large rather than alternating signs.
    golden_angle = np.pi * (3.0 - np.sqrt(5.0))
    specs = list(base_specs)
    for idx in range(len(base_specs), variants_per_reset):
        angle = (idx * golden_angle) % (2.0 * np.pi)
        radius = 0.60 + 0.55 * ((idx * 0.6180339887498949) % 1.0)
        lateral = float(radius * np.cos(angle))
        vertical = float(np.clip(radius * np.sin(angle), -0.85, 1.10))
        ratios = (0.34, 0.68) if idx % 2 == 0 else (0.50,)
        specs.append(
            TrajectoryFamilySpec(
                idx,
                f"detour_{idx:02d}",
...
def _sample_waypoint_set(
    *,
    ...
    family_theta = float(rng.uniform(0.0, 2.0 * np.pi))
    lateral_axis, vertical_axis = _trajectory_basis(start, goal, family_theta)
    ...
    if verbose_waypoints:
        ...
        print(
            f"  [wpt] family {spec.family_id}:{spec.name} "
            f"theta={np.degrees(family_theta):.1f}deg "
            f"path_ratios={ratios_str}",
            flush=True,
        )
...
def _trajectory_basis(
    ...
    if horizontal_norm < 1e-6:
        # Degenerate case: no horizontal direction — use family_theta directly as the axis angle.
        lateral_axis = np.asarray(
            [np.cos(family_theta), np.sin(family_theta), 0.0], dtype=np.float64
        )
    else:
        # Compute the perpendicular-to-approach axis, then rotate it by family_theta about world Z.
        # Each family draws its own family_theta per episode, so no two families share the same
        # lateral plane — breaking any mirror symmetry that would cause diffusion score averaging.
        lateral_axis = np.asarray([-horizontal[1], horizontal[0], 0.0], dtype=np.float64)
        lateral_axis /= np.linalg.norm(lateral_axis[:2])
        cos_r = float(np.cos(family_theta))
        sin_r = float(np.sin(family_theta))
        lx = float(lateral_axis[0])
        ly = float(lateral_axis[1])
        lateral_axis = np.asarray(
            [cos_r * lx - sin_r * ly, sin_r * lx + cos_r * ly, 0.0],
            dtype=np.float64,
        )
...
def _visualize_trajectory_families(args: argparse.Namespace) -> int:
    """Plot sampled waypoint trajectories for all families without the simulator.

    Renders a 3D matplotlib figure showing how each family curves between the
    given start and goal TCP positions. Useful for sanity-checking asymmetry,
    lateral-axis diversity, and curved-path shapes before running data gen.
    """
    try:
    ...
    workspace_bounds = np.asarray(args.workspace_bounds, dtype=np.float64)
    robot_base = np.zeros(3, dtype=np.float64)  # centred at origin for viz
    specs = _trajectory_variant_specs(args.trajectory_variants_per_reset)

    # Each seed gets a different colour family so overlaid seeds are distinguishable.
    ...
    ax.plot(
        ...
        "k--", linewidth=1, alpha=0.4, label="direct",
    )
    ...
    for seed_idx in range(args.viz_seeds):
        ...
        for spec_idx, spec in enumerate(specs):
            ...
            waypoints_arr, _ = _sample_waypoint_set(
                start=start,
                goal=goal,
                spec=spec,
                workspace_bounds=workspace_bounds,
            ...
            if waypoints_arr is None:
                print(f"  [viz] seed {seed_idx} family {spec.name}: no valid waypoints (skipped)")
                continue
            # Build interpolated path: start → waypoints → goal
            path = np.vstack([start[None], waypoints_arr, goal[None]])
            label = f"{spec.name}" if seed_idx == 0 else None
            ax.plot(
                path[:, 0], path[:, 1], path[:, 2],
                color=color, linewidth=1.8, alpha=0.75, label=label,
            )
            ax.scatter(
                waypoints_arr[:, 0], waypoints_arr[:, 1], waypoints_arr[:, 2],
                color=color, s=30, alpha=0.9,
            )
            # Annotate family name on first seed only, near first waypoint
            if seed_idx == 0:
                wp = waypoints_arr[0]
    ...
    ax.set_zlabel("Z (m)")
    ax.set_title(
        f"Trajectory families: start={np.round(start,2)} → goal={np.round(goal,2)}"
        f"\n{len(specs)} families × {args.viz_seeds} seed(s)"
        + (" | curved paths" if args.curved_paths else "")
    )
    ax.legend(loc="upper left", fontsize=7, ncol=2)

    if args.viz_save is not None:
        fig.savefig(args.viz_save, dpi=150, bbox_inches="tight")
        print(f"saved trajectory viz to {args.viz_save}")
    else:
        plt.tight_layout()
        plt.show()
    plt.close(fig)
